...
<div class="container">
    <div class="container-header">
        <div class="header-one">
            @if($playlist->image_path)
                <img src="{{ asset('storage/' . $playlist->image_path) }}" alt="{{ $playlist->name }}" class="img-thumbnail mb-4" width="200">
            @else
                <img src="{{ asset('item/default-image.jpg') }}" alt="No Image" class="img-thumbnail mb-4" width="200">
            @endif
        </div>
        <div class="header-two">
            ...
            <div class="action">
                ...
                <form action="{{ route('user.playlists.destroy', $playlist->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger" onclick="return confirm('Apakah kamu yakin untuk menghapus playlist ini?')">Hapus</button>
                </form>
            </div>
        </div>
    </div>
    ...
    @if($songs->isEmpty())
        <p class="placeholder-no-song pl-12">YAH, belum ada lagu yang anda tambahkan di playlist ini nih, <a href="{{ route('user.playlists.edit', $playlist->id )}}">Tambahkan Sekarang Yuk</a>.</p>
    @else
                ...
                @foreach($songs as $song)
                    <tr onclick="playSelectedSong({{ $loop->index }})">
                        ...
                        <td class="song-title-artist">
                            <img src="{{ $song->image_path ? asset('storage/' . $song->image_path) : asset('item/default-image.jpg') }}" alt="{{ $song->title }}" class="song-img">
                            <div>
                        ...
                        <td>
                            <button class="play-pause-btn" id="play-pause-btn-{{ $loop->index }}" onclick="toggleCardPlayPause(event, {{ $loop->index }})">
